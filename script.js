/* ==========================================================================
   ...
   All data lives in this browser's localStorage. No server, no accounts.
   ========================================================================== */
...
function saveItem(id){
  ...
  const data = {
    ...
    taxRate: parseFloat($('#im-tax').value) || 0,
    pricingMode: $('#im-mode').value,
    commonSizes: editingItemSizes.filter(function(s){ return (Number(s.widthFt) || 0) > 0 && (Number(s.heightFt) || 0) > 0; }),
  };
  if(id){ Object.assign(itemById(id), data); }
  else { state.items.push(Object.assign({ id: uid('itm') }, data)); }
  persist();
  closeModal();
  renderItems();
...
function saveClientFromModal(id){
  ...
  const data = {
    ...
    shipAddress: $('#cm-shipaddress').value.trim(),
    gstin: $('#cm-gstin').value.trim(),
  };
  let savedId = id;
  if(id){ Object.assign(clientById(id), data); }
  else { savedId = uid('cli'); state.clients.push(Object.assign({ id: savedId }, data)); }
  persist();
  closeModal();
...
function saveVendorFromModal(id){
  ...
  let savedId = id;
  if(id){ Object.assign(vendorById(id), data); }
  else { savedId = uid('ven'); state.vendors.push(Object.assign({ id: savedId }, data)); }
  persist();
  closeModal();
...
function saveQuotation(status){
  ...
  if(!qfState.number.trim()){ showToast('Quotation number is required'); return; }
  if(status === 'Sent' && (qfState.status === 'Accepted' || qfState.status === 'Declined')){
    ...
    qfState.status = status;
  }
  if(qfState.id){
    const idx = state.quotations.findIndex(function(q){ return q.id === qfState.id; });
    state.quotations[idx] = qfState;
  } else {
    qfState.id = uid('quo'); qfState.createdAt = Date.now();
    state.quotations.push(qfState);
    state.settings.nextQuotationNumber = (state.settings.nextQuotationNumber || 1) + 1;
  }
  persist();
  showToast(status === 'Draft' ? 'Saved as draft' : 'Quotation saved');
  location.hash = '#quotation-view/' + qfState.id;
}
...
function duplicateQuotation(id){
  ...
  copy.id = uid('quo'); copy.number = nextQuotationNumberPreview();
  state.settings.nextQuotationNumber = (state.settings.nextQuotationNumber || 1) + 1;
  copy.status = 'Draft'; copy.date = todayISO();
  copy.validUntil = addDaysISO(todayISO(), state.settings.defaultValidityDays || 15);
  copy.createdAt = Date.now();
  state.quotations.push(copy);
  persist();
  showToast('Quotation duplicated');
  location.hash = '#quotation-form/' + copy.id;
}
...
function renderQuotationView(id){
  ...
  $('#qv-status-select').addEventListener('change', function(e){ q.status = e.target.value; persist(); renderQuotationView(id); });
...
function saveInvoice(status){
  ...
  if(!hasContent){ showToast('Add at least one line item'); return; }
  if(!ifState.number.trim()){ showToast('Invoice number is required'); return; }
  if(status === 'Sent' && ifState.status === 'Cancelled'){
    /* editing a cancelled invoice: keep it cancelled, just save the edited content */
  } else {
    ifState.status = status;
  }
  if(ifState.id){
    const idx = state.invoices.findIndex(function(i){ return i.id === ifState.id; });
    state.invoices[idx] = ifState;
  } else {
    ifState.id = uid('inv'); ifState.createdAt = Date.now();
    state.invoices.push(ifState);
    state.settings.nextInvoiceNumber = (state.settings.nextInvoiceNumber || 1) + 1;
  }
  persist();
  showToast(status === 'Draft' ? 'Saved as draft' : 'Invoice saved');
  location.hash = '#invoice-view/' + ifState.id;
}
...
function duplicateInvoice(id){
  ...
  copy.id = uid('inv'); copy.number = nextInvoiceNumberPreview();
  state.settings.nextInvoiceNumber = (state.settings.nextInvoiceNumber || 1) + 1;
  copy.status = 'Draft'; copy.date = todayISO();
  copy.dueDate = addDaysISO(todayISO(), state.settings.defaultInvoiceDueDays || 7);
  copy.payments = []; copy.quotationId = null;
  copy.createdAt = Date.now();
  state.invoices.push(copy);
  persist();
  showToast('Invoice duplicated');
  location.hash = '#invoice-form/' + copy.id;
}
...
function renderInvoiceView(id){
  ...
  $('#iv-status-select').addEventListener('change', function(e){ inv.status = e.target.value; persist(); renderInvoiceView(id); });
...
function deletePaymentFromInvoice(invId, paymentId){
...
function convertQuotationToInvoice(quotationId){
  ...
  if(!q) return;
  const inv = {
    id: uid('inv'), number: nextInvoiceNumberPreview(), date: todayISO(),
    dueDate: addDaysISO(todayISO(), state.settings.defaultInvoiceDueDays || 7),
    clientId: q.clientId, reference: q.reference || '', subject: q.subject || '',
    taxType: q.taxType, lines: JSON.parse(JSON.stringify(q.lines)),
    notes: q.notes || '', terms: state.settings.defaultInvoiceTerms || state.settings.defaultTerms,
    status: 'Draft', payments: [], quotationId: q.id, createdAt: Date.now(),
  };
  ...
  location.hash = '#invoice-form/' + inv.id;
}
...
function saveRecordedPayment(){
  ...
  inv.payments.push({ id: uid('pay'), date: $('#pm-date').value || todayISO(), amount: amount, method: $('#pm-method').value, notes: $('#pm-notes').value.trim() });
  persist();
  closeModal();
...
function saveDc(status){
  ...
  if(!hasContent){ showToast('Add at least one line item'); return; }
  if(!dfState.number.trim()){ showToast('Challan number is required'); return; }
  if(status === 'Dispatched' && dfState.status === 'Delivered'){
    ...
    dfState.status = status;
  }
  if(dfState.id){
    const idx = state.deliveryChallans.findIndex(function(d){ return d.id === dfState.id; });
    state.deliveryChallans[idx] = dfState;
  } else {
    dfState.id = uid('dc'); dfState.createdAt = Date.now();
    state.deliveryChallans.push(dfState);
    state.settings.nextDcNumber = (state.settings.nextDcNumber || 1) + 1;
  }
  persist();
  showToast(status === 'Draft' ? 'Saved as draft' : 'Delivery challan saved');
  location.hash = '#dc-view/' + dfState.id;
}
...
function duplicateDc(id){
  ...
  if(!dc) return;
  const copy = JSON.parse(JSON.stringify(dc));
  copy.id = uid('dc'); copy.number = nextDcNumberPreview();
  state.settings.nextDcNumber = (state.settings.nextDcNumber || 1) + 1;
  copy.status = 'Draft'; copy.date = todayISO();
  copy.vehicleNumber = ''; copy.transportMode = ''; copy.invoiceId = null;
  copy.createdAt = Date.now();
  state.deliveryChallans.push(copy);
  persist();
  showToast('Delivery challan duplicated');
  location.hash = '#dc-form/' + copy.id;
}
...
function renderDcView(id){
  ...
  $('#dv-status-select').addEventListener('change', function(e){ dc.status = e.target.value; persist(); renderDcView(id); });

  $('#dc-print-doc').innerHTML = deliveryChallanPrintHtml(dc);
}

function convertInvoiceToDeliveryChallan(invoiceId){
  const inv = state.invoices.find(function(x){ return x.id === invoiceId; });
  if(!inv) return;
  const dc = {
    id: uid('dc'), number: nextDcNumberPreview(), date: todayISO(),
    vehicleNumber: '', transportMode: '', invoiceId: inv.id,
    clientId: inv.clientId, reference: inv.reference || '', subject: inv.subject || '',
    taxType: inv.taxType, lines: JSON.parse(JSON.stringify(inv.lines)),
    notes: inv.notes || '', terms: state.settings.defaultDcTerms,
    status: 'Draft', adjustment: 0, createdAt: Date.now(),
  };
  state.deliveryChallans.push(dc);
  state.settings.nextDcNumber = (state.settings.nextDcNumber || 1) + 1;
  persist();
  showToast('Delivery challan created from invoice');
  location.hash = '#dc-form/' + dc.id;
}
...
function savePo(status){
  ...
  if(!hasContent){ showToast('Add at least one line item'); return; }
  if(!pfState.number.trim()){ showToast('PO number is required'); return; }
  if(status === 'Sent' && (pfState.status === 'Received' || pfState.status === 'Cancelled')){
    ...
    pfState.status = status;
  }
  if(pfState.id){
    const idx = state.purchaseOrders.findIndex(function(p){ return p.id === pfState.id; });
    state.purchaseOrders[idx] = pfState;
  } else {
    pfState.id = uid('po'); pfState.createdAt = Date.now();
    state.purchaseOrders.push(pfState);
    state.settings.nextPoNumber = (state.settings.nextPoNumber || 1) + 1;
  }
  persist();
  showToast(status === 'Draft' ? 'Saved as draft' : 'Purchase order saved');
  location.hash = '#po-view/' + pfState.id;
}
...
function duplicatePo(id){
  ...
  copy.id = uid('po'); copy.number = nextPoNumberPreview();
  state.settings.nextPoNumber = (state.settings.nextPoNumber || 1) + 1;
  copy.status = 'Draft'; copy.date = todayISO(); copy.expectedDate = '';
  copy.createdAt = Date.now();
  state.purchaseOrders.push(copy);
  persist();
  showToast('Purchase order duplicated');
  location.hash = '#po-form/' + copy.id;
}
...
function renderPoView(id){
  ...
  $('#pv-status-select').addEventListener('change', function(e){ po.status = e.target.value; persist(); renderPoView(id); });
...
function saveExpense(id){
  ...
  const data = {
    ...
    vendorId: $('#em-vendor').value,
    amount: amount,
    description: $('#em-desc').value.trim(),
    paymentMethod: $('#em-method').value,
    notes: $('#em-notes').value.trim(),
  };
  if(id){ Object.assign(state.expenses.find(function(e){ return e.id === id; }), data); }
...
function onSaveSettings(e){
  ...
  Object.assign(state.settings, {
    ...
    quotationPrefix: $('#set-prefix').value.trim() || 'CC-QT-',
    nextQuotationNumber: parseInt($('#set-nextnum').value, 10) || 1,
    defaultValidityDays: parseInt($('#set-validity').value, 10) || 15,
    defaultTerms: $('#set-terms').value,
    invoicePrefix: $('#set-inv-prefix').value.trim() || 'CC-INV-',
    nextInvoiceNumber: parseInt($('#set-inv-nextnum').value, 10) || 1,
    defaultInvoiceDueDays: parseInt($('#set-inv-duedays').value, 10) || 7,
    defaultInvoiceTerms: $('#set-inv-terms').value,
    poPrefix: $('#set-po-prefix').value.trim() || 'CC-PO-',
    nextPoNumber: parseInt($('#set-po-nextnum').value, 10) || 1,
    defaultPoTerms: $('#set-po-terms').value,
    dcPrefix: $('#set-dc-prefix').value.trim() || 'CC-DC-',
    nextDcNumber: parseInt($('#set-dc-nextnum').value, 10) || 1,
    defaultDcTerms: $('#set-dc-terms').value,
  });
  persist();
...
function onImportBackup(e){
  ...
  reader.onload = function(){
    try{
      ...
      if(!confirm('Import this backup? It will replace all current data in this browser.')){ e.target.value = ''; return; }
      state.items = data.items || state.items;
      state.clients = data.clients || state.clients;
      state.vendors = data.vendors || state.vendors;
      state.quotations = data.quotations || state.quotations;
      state.invoices = data.invoices || state.invoices;
      state.expenses = data.expenses || state.expenses;
      state.purchaseOrders = data.purchaseOrders || state.purchaseOrders;
      state.deliveryChallans = data.deliveryChallans || state.deliveryChallans;
...
function onClearData(){
  if(!confirm('This permanently deletes ALL data in this browser. Continue?')) return;
  if(!confirm('Are you absolutely sure? This cannot be undone.')) return;
  state.items = DEFAULT_ITEMS.map(function(i){ return Object.assign({}, i); });
  state.clients = []; state.vendors = [];
  state.quotations = []; state.invoices = []; state.expenses = []; state.purchaseOrders = []; state.deliveryChallans = [];
